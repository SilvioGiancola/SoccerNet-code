...
class FeatureExtractorResNet(FeatureExtractorBase):


	def __init__(self):
		logging.info('FeatureExtractorResNET Initialization')
		logging.debug('create the ResNET network from Keras or whatever')
		self.TFResource = ''
		self.featureName = 'ResNET'
		self.model = resnet152_model('weights_resnet152_imageNet_tf.h5')
		return
	...
	def elaborateVideo(self, videoPath, game_start_time, game_end_time):
		...
		logging.debug('infer the ResNET Model on the videos in gamePath')
		logging.info('Working in ResNET model')
		...
		logging.debug('Converting the Video into ResNET')
		layer_name = 'Flatten'
		intermediate_layer_model = Model(inputs= self.model.input, outputs = self.model.get_layer(layer_name).output)

		frame = []
		for index in np.arange(0, len(vid)):
			if (int(index)%(fps*self.intervalStrideTime) < 1):
				frame.append(vid[int(index)])
		
		logging.info( 'Converting Selected Frames Into ResNET Representation ...' )
		start_time__half_elaboration = time.time()
		frame = np.array(frame)
		features_array = intermediate_layer_model.predict(frame)
		logging.info(' Resnet Representation Successfully Obtained in '+ str((time.time() - start_time__half_elaboration)/60) + ' min' )
